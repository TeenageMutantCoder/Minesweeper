...
class Game():
    def __init__(self, width, height, num_rows=8, num_columns=8, difficulty="easy"):
        pygame.init()
        self.width = width
        self.height = height
        ...
        self.box_size_x = self.width // self.num_columns
        self.box_size_y = self.height // self.num_rows

        # Make game surface only as big as it needs to be so that it can be centered
        game_surface_width = self.box_size_x * self.num_columns
        game_surface_height = self.box_size_y * self.num_rows
        ...
        self.flag_icon = pygame.transform.scale(self.flag_icon, (self.box_size_x - 1, self.box_size_y - 1))
        self.mine_icon = pygame.transform.scale(self.mine_icon, (self.box_size_x - 1, self.box_size_y - 1))
    ...
    def start_game(self):
        # Create grid of size given by variables self.num_rows and self.num_columns
        self.grid = [[Square() for count_y in range(self.num_columns)] for count_x in range(self.num_rows)]
    ...
    def find_grid_position(self, pos_x, pos_y):
        ...
        for count_x in range(self.num_columns):
            if (count_x * self.box_size_x) < pos_x < ((count_x + 1) * self.box_size_x):
                column = count_x
                break
        for count_y in range(self.num_rows):
            if (count_y * self.box_size_y) < pos_y < ((count_y + 1) * self.box_size_y):
                row = count_y
                break
        try:
            return row, column
    ...
    def check_nearby_squares(self, row, column):
        ...
        if row == 0 and column == 0:  # Upper left corner grid square
            ...
            return
        elif row == 0 and column == self.num_columns - 1:  # Upper right corner grid square
            check_left(self, row, column)
            check_lower_left(self, row, column)
            check_down(self, row, column)
            return
        elif row == self.num_rows - 1 and column == 0:  # Lower left corner grid square
            check_up(self, row, column)
            check_upper_right(self, row, column)
            check_right(self, row, column)
            return
        elif row == self.num_rows - 1 and column == self.num_columns - 1:  # Lower right corner grid square
            ...
            check_lower_right(self, row, column)
            return
        elif row == self.num_rows - 1:  # Bottom side
            check_upper_left(self, row, column)
            check_up(self, row, column)
            check_upper_right(self, row, column)
            check_left(self, row, column)
            ...
            check_lower_right(self, row, column)
            return
        elif column == self.num_columns - 1: # Right side
            check_upper_left(self, row, column)
            check_up(self, row, column)
            check_left(self, row, column)
            check_lower_left(self, row, column)
    ...
    def draw_grid(self, row, column):
        rect = pygame.Rect((column * self.box_size_x, row * self.box_size_y), (self.box_size_x, self.box_size_y))
        pygame.draw.rect(self.game_surface, (0, 0, 0), rect, 1)
    ...
    def show_num_of_nearby_mines(self, row, column):
        ...
        rect.center = (column * self.box_size_x + self.box_size_x // 2, row * self.box_size_y + self.box_size_y // 2)
        self.game_surface.blit(nearby_mine_count_text, rect)

    def show_clicked(self, row, column):
        square = self.grid[row][column]
        rect = pygame.Rect((column * self.box_size_x, row * self.box_size_y), (self.box_size_x - 1, self.box_size_y - 1))
    ...
    def show_flagged(self, row, column):
        rect = pygame.Rect((column * self.box_size_x, row * self.box_size_y), (self.box_size_x - 1, self.box_size_y - 1))
        self.game_surface.blit(self.flag_icon, rect)
        ...
        for row in range(self.num_rows):
            for column in range(self.num_columns):
                if self.grid[row][column].is_mine:
                    rect = pygame.Rect((column * self.box_size_x, row * self.box_size_y), (self.box_size_x - 1, self.box_size_y - 1))
                    pygame.draw.rect(self.game_surface, (135, 0, 0), rect)
                    self.game_surface.blit(self.mine_icon, rect)
    ...
    def update(self):
        ...
        while running:
            ...
            if not self.game_over:
                ...
                self.game_surface.blit(game_over_message, rect)

            rect = self.game_surface.get_rect()
            rect.center = (self.width // 2, self.height // 2)
            self.window.fill((255, 255, 255))
            self.window.blit(self.game_surface, rect)
            pygame.display.flip()


if __name__ == "__main__":
    Game(700, 700, num_rows=8, num_columns=8, difficulty="easy")